...
func (self *DailyBillController) BatchPay(ctx *iris.Context) {
	...
	for _, _param := range paramList {
		...
		aliPayBillMap, err1 := dailyBillService.BasicMap(billAt, 1, userIds...)  //查询出支付宝结算方式中"已申请"的账单(后台自动将未结算改成已申请)
		aliPayFailureBillMap, err2 := dailyBillService.BasicMap(billAt, 4, userIds...)   //查询出支付宝结算方式中失败的账单
		if err2 == nil && len(aliPayFailureBillMap)>0 {
			for k, v := range aliPayFailureBillMap {
				aliPayBillMap[k] = v
			}
		}
		if err1 == nil && err2 == nil && len(aliPayBillMap) > 0 {
			for _userId, _ := range aliPayBillMap {
...
func (self *DailyBillController) Notification (ctx *iris.Context) {
	...
	if billLength != (succeedNum+failureNum) {
		//ctx.JSON(iris.StatusOK, &enity.Result{"01060501", nil, daily_bill_msg["01060501"]})
		ctx.Render("batch_alipay_notify.html", map[string]interface{}{"msg": "fail"})
		return
	}
	rows, err := dailyBillService.Updates(&billList)
	...
	if billLength != rows {   //更新支付宝成功订单部分失败
		//ctx.JSON(iris.StatusOK, &enity.Result{"01060501", nil, daily_bill_msg["01060501"]})
		ctx.Render("batch_alipay_notify.html", map[string]interface{}{"msg": "fail"})
		return
	}
	...
	common.Log(ctx, nil)
	//ctx.JSON(iris.StatusOK, &enity.Result{"01060500", nil, daily_bill_msg["01060500"]})
