...
func (self *DailyBillExport) Excel(cashAccountType int, status []string, userId int, searchStr string, roleId int, startAt string, endAt string) (string, error) {
	...
	rows, err := common.SodaMngDB_R.Table("daily_bill bill").
		Select("bill.id,bill.user_name,bill.account_name,bill_at,order_count,status,REPLACE(REPLACE(REPLACE(bill.`account`,' ',''),'*',''),'-','') AS account,bill.real_name,bill.bank_name,p.name as province,c.name as city,total_amount,cash.head_bank_name").
