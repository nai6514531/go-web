...
const App = React.createClass({
        ...
        render: (id, record) => {
          ...
          switch (roleId) {
            case 1:
              spanDiv = (
                <a
                  href={`#settlement/daily-bill-detail/${record.userId}/${moment(record.billAt).format('YYYY-MM-DD')}`}>明细</a>
              )
              break;
            case 2:
            case 5:
  ...
  render(){
    ...
    return (<section className="view-settlement-list">
      ...
      <Table rowSelection={rowSelection} dataSource={list} columns={columns} pagination={pagination} bordered
             loading={this.state.loading} footer={() => footer}/>
      <Modal
        title="支付二次确认"
